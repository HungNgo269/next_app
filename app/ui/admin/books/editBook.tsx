...
import { EditBookAction } from "@/app/(admin)/dashboard/books/editBookActions";
...
export default function EditBook({ book }: Props) {
  ...
  const [formData, setFormData] = useState({
    ...
    is_active: Boolean(book?.is_active) ?? true,
  });
  ...
  useEffect(() => {
    if (isDialogOpen && book) {
      ...
      setImagePreview(book.image_urls?.[0] ?? null);
